...
/** "Grand BallRoom 1" -> "grand-ballroom-1", matching the venue slugs in the DB. */
...
export default function ConferenceEventsClient({
    venueSlugs = [],
}: {
    /** Slugs that have a live detail page, so cards only link where one exists. */
    venueSlugs?: string[];
}) {
    const availableSlugs = new Set(venueSlugs);
                ...
                <section className="py-8 md:py-10 px-6 md:px-10 scroll-mt-[130px] md:scroll-mt-[150px]">
                    <div className="max-w-[1600px] mx-auto">
                        <div className="text-center mb-10 md:mb-12">
                            ...
                            {/* h1: primary heading for the conference & events hub. */}
                            <h1 className="text-4xl md:text-5xl font-serif text-[#2C3632] tracking-tight">Spaces Designed for Business Flow</h1>
                        </div>
                        <div className="space-y-8 md:space-y-10">
                            {mainVenues.map((venue, index) => (
                                <VenueCard
                                    ...
                                    detailHref={
                                        availableSlugs.has(venue.key)
                                            ? `/conference-events/${venue.key}`
                                            : undefined
                                    }
                                    subVenueHref={(name) => {
                                        const slug = slugify(name);
                                        return availableSlugs.has(slug)
                                            ? `/conference-events/${slug}`
                                            : undefined;
                                    }}
                                />
                            ))}
...
function VenueCard({
    ...
    venue: typeof mainVenues[0];
    index: number;
    imageUrl?: string;
    detailHref?: string;
    subVenueHref?: (name: string) => string | undefined;
}) {
            ...
            <div className={`w-full lg:w-1/2 flex flex-col justify-center py-2 lg:py-0 ${isEven ? 'lg:pl-8' : 'lg:items-end lg:pr-8 text-right'}`}>
                <div className="max-w-lg">
                        ...
                        <div className="mb-6 border border-[#E6DDCE] bg-[#FCFAF5] px-4 py-3">
                            ...
                            <div className="space-y-2">
                                {venue.subVenues.map((sub) => {
                                    const href = subVenueHref?.(sub.name);
                                    return (
                                        <div key={sub.name} className="flex items-start justify-between gap-3 text-xs">
                                            {href ? (
                                                <Link
                                                    href={href}
                                                    className="text-[#403A35] font-medium whitespace-nowrap underline decoration-[#C6AF84] underline-offset-4 hover:text-[var(--brand-primary-dark)] transition-colors"
                                                >
                                                    {sub.name}
                                                </Link>
                                            ) : (
                    ...
                    {detailHref && (
                        <Link
                            href={detailHref}
                            className="inline-flex items-center text-[11px] tracking-[0.24em] uppercase text-[var(--brand-primary-dark)] underline decoration-[#C6AF84] underline-offset-[6px] hover:text-black transition-colors"
                        >
                            View Venue Details
                        </Link>
                    )}
